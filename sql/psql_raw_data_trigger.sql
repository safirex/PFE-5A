...
CREATE or REPLACE FUNCTION insert_trigger() RETURNS trigger AS $$
    BEGIN
        ...
        VALUES (new.trip_id,new.route_id,new.direction_id,to_timestamp(new.timestamp)::date,new.vehicle_id,new.vehicle_label);
        END if;
        ...
        VALUES ( new.trip_id,new.stop_id,new.arrival_delay,to_timestamp(new.arrival_time)::date,new.departure_delay,to_timestamp(new.departure_time)::date,NEW.scheduled_relationship);
        END if;
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
