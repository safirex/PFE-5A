...
CREATE or REPLACE FUNCTION insert_trigger() RETURNS trigger AS $$
    BEGIN
        ...
        VALUES (new.trip_id,new.route_id,new.direction_id,to_timestamp(new.timestamp),new.vehicle_id,new.vehicle_label);
        END if;
        ...
        VALUES ( new.trip_id,new.stop_id,new.arrival_delay,to_timestamp(new.arrival_time),new.departure_delay,to_timestamp(new.departure_time),NEW.scheduled_relationship);
        END if;
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
